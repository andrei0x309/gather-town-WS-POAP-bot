...
class GatherPOAPBot {
  constructor(DBLocation) {
    ...
    this.ethChallengeStore = new new Conf({ configName: 'eth-challenge', schema: ethChallengeSchema , cwd: DBLocation })
  ...
  async onCheckCommand(msg, context) {
    if (msg.playerChats.contents.includes('/bot')) {
      ...
      if (this.authorizedUsers.includes(msg.playerChats.senderId)) {
        ...
        switch (command) {
          case 'link-eth':
            const users = ethLinksStore.get('users')
            if (users.find((u) => u.id === user.id)) {
              this.game.chat(`${user.id}`, [], user.map, 'Your account alread has an eth attached')
            } else {
            ...
            challenges.push({
              code,
              user: user.id
              mesage: "Please sign to this message link: " + code
              expiration
            })
            this.ethChallengeStore.set('challenges', challenges)
            this.game.chat(`${user.id}`, [], user.map, `Please Link your eth using this link: ${this.backendApi}/${code} will expire in 15 minutes`)
           }
            break
            default:
            this.game.chat(`${user.id}`, [], user.map, 'Command not found')
