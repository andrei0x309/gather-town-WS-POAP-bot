...
class GatherPOAPBot {
    ...
    constructor( apiKey = API_KEY, gatherSpace = GATHER_SPACE, debugLog = true ) {
        ...
        this.game.subscribeToConnection(this.onConnected);
        this.game.subscribeToEvent("playerJoins", (player) => {
            if(this.debugLog) console.log("Player Joins Event:", player);
        });
        this.game.subscribeToEvent("playerChats", this.onCheckCommand)

    }

    onCheckCommand = async (msg, context) => {
        // if(this.debugLog) console.log("Command:", msg, context);
                ...
                switch(command){
                    case 'send-poap':
                       if(this.usersPOAPSent.length > 0){
                            this.game.chat(`${msg.playerChats.senderId}`, [], this.game.players[msg.playerChats.senderId].map, `POAP ALREADY SENT USE: /bot poap-new-users`);
                        }else {
                           this.sendPoapToUsers();
                        }
                        break;
                    ...
                    case 'disconnect':
                        ...
                        break;
                    default:
                        this.game.chat(`${msg.playerChats.senderId}`, [], this.game.players[msg.playerChats.senderId].map, `Command not found`);
                }

            }else{
                this.game.chat(`${msg.playerChats.senderId}`, [], this.game.players[msg.playerChats.senderId].map, `You are not authorized to use this bot.`);
            }
        }
    }
